...
Therefore, the optimal discriminator is $D^*(x) = \frac{p_{data}(x)}{p_{data}(x) + p_{g}(x)}$

**Theorem 2: Give an optimal discriminator $D^*$, the optimal generator $G^*$, which aims to minimize the objective function, is obtained when $p_g = p_{data}$**
...
V(D^*, G) & = \mathbb{E}_{x \sim p_{data}}[\log D^*(x)] + \mathbb{E}_{z \sim p_z}[\log (1 - D^*(G(z)))] \\
 ...
 & =  \mathbb{E}_{x \sim p_{data}}[\log(\frac{p_{data}(x)}{p_{data}(x) + p_g(x)})] + \mathbb{E}_{x \sim p_{g}}[\log (\frac{p_g(x)}{p_{data}(x) + p_g(x)})] \\
 & =  \mathbb{E}_{x \sim p_{data}}[\log(\frac{p_{data}(x)}{p_{data}(x) + p_g(x)})] + \mathbb{E}_{x \sim p_{g}}[\log (\frac{p_g(x)}{p_{data}(x) + p_g(x)})] \\
